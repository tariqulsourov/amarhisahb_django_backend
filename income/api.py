...
from income.serializers import IncomeRelatedSerializer, IncomesSerializer
from savings.models import Wallet, MonthlyBalanceSummary

class IncomeRelatedViewSet(viewsets.ModelViewSet):
    serializer_class = IncomeRelatedSerializer
    permission_classes = [permissions.IsAuthenticated]

    def get_queryset(self):
        return IncomeRelated.objects.filter(create_by=self.request.user)
    ...
    def perform_destroy(self, instance):
        ...
        default_cat, created = IncomeRelated.objects.get_or_create(
            create_by=user,
            short_info="System Default Income Category",
        ...
        # Reassign all incomes using this category to the default category
        Incomes.objects.filter(income_related_id=instance).update(income_related_id=default_cat)
        instance.delete()
...
class IncomesViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Incomes.objects.filter(income_related_id__create_by=self.request.user).order_by('-income_date', '-id')
    ...
    def perform_create(self, serializer):
        ...
        income = serializer.save(wallet=wallet)

        # 3. Update Monthly Balance Summary
        income_date = income.income_date or date.today()
        year, month = income_date.year, income_date.month
        if month == 2:
            try:
    ...
    def perform_update(self, serializer):
        ...
        # Save update
        income = serializer.save(wallet=target_wallet)

        # Update Monthly Summary if amount changed
    ...
    def perform_destroy(self, instance):
        ...
        income_date = instance.income_date or date.today()
        year, month = income_date.year, income_date.month
        if month == 2:
            try:
                last_date_of_month = datetime(year, month, 28).date()
